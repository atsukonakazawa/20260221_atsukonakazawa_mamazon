...
- **販売会社**：商品の登録申請（仮登録）
- **管理者**：販売会社が登録申請した商品の承認をはじめ、商品管理、ユーザー管理、販売会社管理、注文管理
...
Laravel と Next.js を組み合わせた API 開発を実践的に学ぶことを目的として、本アプリを制作しました。

EC サイトで一般的な商品管理、注文管理、レビュー、画像アップロードなどの機能を一つずつ追加しながら、フロントエンドとバックエンドの連携やデータベース設計について学びました。

また、実務を意識し、Laravel を API サーバー、Next.js をフロントエンドとする構成で開発しました。Cloudinary を利用した画像アップロード機能や、Stripe（テストモード）を利用したクレジットカード決済機能を実装しています。

さらに、本番環境へのデプロイには Railway と Vercel を利用しました。
